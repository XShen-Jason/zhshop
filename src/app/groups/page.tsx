            ...
            <Link href={`/groups/${g.id}`}>
                <Card className="group flex flex-col h-full">
                    <div className="p-6 flex-1 flex flex-col cursor-pointer">
                        ...
                        <h3 className="text-xl font-bold text-gray-900 mb-2 group-hover:text-indigo-600 transition-colors flex items-center gap-2">
                            {/* Extract base title and batch number */}
                            {g.title.replace(/ #\d+$/, '')}
                            {/ #(\d+)$/.test(g.title) && (
                                <span className="inline-flex items-center justify-center min-w-[22px] h-[22px] px-1.5 bg-indigo-100 text-indigo-600 text-xs font-bold rounded-full">
                                    {g.title.match(/ #(\d+)$/)?.[1]}
                                </span>
                            )}
                        </h3>
