...
export default function UserPage() {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            // Auto-redirect to Profile for fresh users (only welcome bonus)
            // Heuristic: 100 points (welcome), no orders, no lotteries, no groups
            if ((data.orders || []).length === 0 &&
                (data.lotteries || []).length === 0 &&
                (data.groups || []).length === 0 &&
                data.profile?.points === 100 &&
                !searchParams.get('tab') // Only if not explicitly navigating
            ) {
                setActiveTab('contacts');
            }
        } catch (error) {
