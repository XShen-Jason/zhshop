...
export async function GET() {
    try {
        ...
        // Fetch all group participations for this user with quantity data
        ...
        for (const row of data || []) {
            ...
            if (!aggregated[gid]) {
                ...
                const allParticipants = groupData?.group_participants || [];
                const liveCount = allParticipants.reduce((sum: number, p: any) => sum + (p.quantity || 1), 0);
...
export async function PUT(request: Request) {
    try {
        ...
        // Get the single entry for this user (unique constraint ensures only one)
            ...
            if (existingEntry) {
                await supabase
                    ...
                    .update({
                        ...
                        contact_info: contactInfo || existingEntry.contact_info
                    })
