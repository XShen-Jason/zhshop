...
export default function AdminPage() {
        ...
        try {
            ...
            if (res.ok) {
                ...
                setSiteConfig({
                    ...
                    qq_group_link: data.qq_group_link || '',
                    official_email: data.official_email || '',
                    friend_links: data.friend_links || '',
                    footer_copyright: data.footer_copyright || '',
                    footer_qr_image: data.footer_qr_image || '',
                    footer_qr_title: data.footer_qr_title || ''
                });
            }
        } catch (e) {
    ...
    const handleBatchRename = async (type: 'category' | 'subCategory', oldName: string, newName: string, parentCategory?: string) => {
        const confirmed = await confirm({
            message: `确定将 "${oldName}" 重命名为 "${newName}" 吗？所有相关商品都将更新。`,
            variant: 'warning',
            confirmText: '确定重命名'
        });
        if (!confirmed) return;
    ...
    const handleUpdateGroupStatus = async (id: string, newStatus: string) => {
        const confirmed = await confirm({
            message: '确定要更新拼团状态吗？',
            variant: 'warning'
        });
    ...
    const executeDraw = async (id: string) => {
        ...
        } catch (e) { console.error(e); showToast('网络错误，请稍后重试', 'error'); }
    };
    ...
    const handleParticipantDelete = async (userId: string) => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                viewParticipants(currentViewType!, currentViewId);
            } else {
        ...
        <div className="space-y-6">
            ...
            <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Telegram 群链接</label>
                        <input
                            type="url"
                    ...
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">QQ 群号</label>
                        <input
                            type="text"
                    ...
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">官方邮箱</label>
                        <input
                            type="text"
                    ...
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">页脚版权文字</label>
                        <input
                            type="text"
                    ...
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">二维码图片</label>
                        <div className="flex items-center gap-3">
                            <div className="relative w-16 h-16 border rounded bg-gray-50 flex items-center justify-center overflow-hidden">
                            ...
                            <div className="flex-1">
                                <label className="cursor-pointer bg-white border border-gray-300 text-gray-700 px-3 py-1.5 rounded text-sm hover:bg-gray-50 inline-flex items-center">
                                ...
                                <p className="text-xs text-gray-500 mt-1">支持 jpg, png, gif, webp (max 5MB)</p>
                            </div>
                        </div>
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">二维码标题</label>
                        <input
                            type="text"
                    ...
                    <div className="md:col-span-2">
                        <label className="block text-sm font-medium text-gray-700 mb-1">友情链接 (名称|链接 每行一个)</label>
                        <textarea
                            ...
                            rows={3}
                            className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-transparent font-mono"
                        />
                    </div>
                </div>
                <button
                    ...
                    className="mt-4 bg-indigo-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-indigo-700 disabled:opacity-50 flex items-center"
                >
                    {siteConfigLoading ? <Loader2 className="animate-spin mr-2" size={16} /> : <Save size={16} className="mr-2" />}
                    保存配置
                </button>
            </div>
        </div>
    ...
    const renderOrders = () => {
        ...
        const OrderCard = ({ order }: { order: Order }) => (
            <div className="bg-white border rounded-lg p-3 shadow-sm hover:shadow-md transition-all text-sm group relative">
                ...
                {order.quantity && <div className="text-xs font-bold text-amber-600 mb-2 flex items-center gap-1">
                    <Box size={12} />
                    数量: {order.quantity}
                </div>}
                ...
                {/* Price display */}
                {order.cost && (
                    <div className="text-xs text-gray-600 mb-2 flex items-center justify-between bg-gray-50 p-2 rounded">
                        {order.quantity && order.quantity > 1 ? (
                            <>
                                <span>单价: ￥{(order.cost / order.quantity).toFixed(2)}</span>
                                <span className="font-bold text-indigo-600">总价: ￥{order.cost.toFixed(2)}</span>
                            </>
                        ) : (
                            <span className="font-bold text-indigo-600">价格: ￥{order.cost.toFixed(2)}</span>
                        )}
                    </div>
                )}
                        ...
                        <div key={g.id} className="bg-white p-4 rounded-xl shadow-sm border border-gray-200 hover:shadow-md transition-all group">
                            <div className="flex justify-between items-start mb-2">
                                <div className="flex items-center gap-2 flex-1 min-w-0">
                                    <h5 className="font-bold text-gray-800 truncate" title={g.title}>
                                        {g.title.replace(/ #\d+$/, '')}
                                    </h5>
                                    {/ #(\d+)$/.test(g.title) && (
                                        <span className="inline-flex items-center justify-center min-w-[20px] h-[20px] px-1 bg-indigo-100 text-indigo-600 text-[10px] font-bold rounded-full shrink-0">
                                            {g.title.match(/ #(\d+)$/)?.[1]}
                                        </span>
                                    )}
                                    {g.autoRenew && (
                                ...
                                <span className={`text-xs px-2 py-0.5 rounded-full shrink-0 ${g.status === '进行中' ? 'bg-green-100 text-green-800' :
                            ...
                            <div className="flex border-t pt-3 gap-2">
                                ...
                                {type === 'LOCKED' && (
                                    <button
                                        onClick={() => handleUpdateGroupStatus(g.id, '已结束')}
        ...
        const GroupCard = ({ group }: { group: GroupBuy }) => (
            <div key={group.id} className="bg-white border rounded-lg p-3 shadow-sm hover:shadow-md transition-shadow relative group">
                <div className="flex justify-between items-start mb-2 gap-2">
                    ...
                    <button onClick={() => { setEditingGroup(group); setShowGroupModal(true); }} className="text-gray-400 hover:text-indigo-600 shrink-0"><Edit size={14} /></button>
                </div>
                ...
                <div className="flex justify-between gap-1 mt-2">
                    {group.status === '进行中' && (
                        <button onClick={() => handleUpdateGroupStatus(group.id, '已锁单')} className="flex-1 bg-yellow-50 text-yellow-600 py-1 rounded text-xs hover:bg-yellow-100">
                            锁单
                        </button>
                    )}
                    {group.status === '已锁单' && (
                        <>
                            <button onClick={() => handleUpdateGroupStatus(group.id, '进行中')} className="flex-1 bg-green-50 text-green-600 py-1 rounded text-xs hover:bg-green-100">
                                恢复
                            </button>
                            <button onClick={() => handleUpdateGroupStatus(group.id, '已结束')} className="flex-1 bg-gray-100 text-gray-600 py-1 rounded text-xs hover:bg-gray-200">
                                结束
                            </button>
                        </>
                    )}
        ...
        const renderLotteryTable = (items: Lottery[], isPending: boolean) => (
            <div className="bg-white rounded shadow text-sm overflow-hidden border border-gray-200">
                <table className="w-full text-left">
                    <thead className="bg-gray-50"><tr><th className="p-3">标题</th><th className="p-3">日期</th><th className="p-3">参与情况</th><th className="p-3">操作</th></tr></thead>
                                    ...
                                    <td className="p-3">
                                        <div className="text-xs space-y-1">
                                            <div className="flex items-center gap-1">
                                                ...
                                                <span className={`font-bold ${l.participants >= (l.minParticipants || 0) ? 'text-green-600' : 'text-red-600'}`}>
                                                    {l.participants}
                                                </span>
                                                <span className="text-gray-400">/</span>
                                                <span className="text-gray-600">{l.minParticipants || 0}</span>
                                            </div>
                                            <div className="flex items-center gap-1">
                                                <Gift size={12} className="text-purple-600" />
                                                <span className="text-gray-600">中奖: {l.winnersCount}人</span>
                                            </div>
                                            <button onClick={() => viewParticipants('LOTTERY', l.id)} className="text-indigo-600 hover:underline">查看名单</button>
                                        </div>
                                    </td>
